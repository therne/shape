...
public class Parser extends BaseParser {
    ...
    void parseDirectionalAttribute(Node node, String key, Token firstValue) throws SyntaxError {
        // first, add top
        ArrayList<Token> values = new ArrayList<Token>(4),
                units = new ArrayList<Token>(4);
    ...
    void handleOtherAttribute(Node node, String attrKey) throws SyntaxError {
        ...
        if (attrKey.equals("font")) {
            attr = new FontAttribute(value);

        } else if (attrKey.equals("width") || attrKey.equals("height")) {
            attr = new LayoutParam(attrKey, value);

        } else if (attrKey.equals("weight")) {
    ...
    void handleStateAttribute() throws SyntaxError {
        ...
        String s = currToken.getLexeme();
        if (s.equals("vertical")) {
            attr = new Attribute("orientation", "vertical");
        } else if (s.equals("horizontal")) {
            attr = new Attribute("orientation", "horizontal");
        } else if (s.equals("editable")) {
            attr = new Attribute("editable", "true");
        } else if (s.equals("visible")) {
            attr = new Attribute("visibility", "visible");
        } else if (s.equals("invisible")) {
            attr = new Attribute("visibility", "invisible");
        } else if (s.equals("focusable")) {
            attr = new Attribute("focusable", "true");
        } else if (s.equals("clickable")) {
            attr = new Attribute("clickable", "true");
        } else {
            throw new SyntaxError("no state attribute found : " + currToken.getLexeme(), currToken);
        }
